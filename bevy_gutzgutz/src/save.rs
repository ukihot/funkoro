//! `GutzSavePlugin` — 「セーブファイル」ではなく「ゲーム進行状態の永続化」。
//!
//! Saveはゲーム進行状態をディスクへ出し入れするための**インフラ**であり、
...
//! `#[derive(SettingsGroup)]`）を使うこと。`GutzSaveData`で音量等を
//! 表現することも技術的には可能だが、意味的に別物（「ハイスコアやステージ
//! 進行はゲームの状態」に対し「音量はプレイヤー環境の好み」）な上、Bevy側が
//! 同じ問題（読み込み・保存・保存先解決）を公式にカバーするようになった以上、
//! gutzgutzが薄いラッパーを重ねる価値はない（Bevy本体を薄くラップしない、
//! という既存方針と同じ理由）。`GutzSaveData`は「ゲームを再開したときに
//! 復元したい進行データ」だけに使うこと。
//!
